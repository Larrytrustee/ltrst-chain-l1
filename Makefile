...
ifeq (,$(VERSION))
  ...
  ifeq (,$(VERSION))
    VERSION := dev-$(shell echo $(COMMIT) | cut -c1-8)
  endif
endif

# ldflags injected into the binary — stamp app, version, commit.
# -checklinkname=0 is required because bytedance/sonic (pulled in via gin) uses
# go:linkname against encoding/json.unquoteBytes, which Go 1.26 rejects without
# this flag. Drop -checklinkname=0 once sonic ships a Go-1.26-compatible release.
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=$(APPNAME) \
	-X github.com/cosmos/cosmos-sdk/version.AppName=$(DAEMON) \
